...
void SyscallDispatcher() 
{
	...
	if (index < MAX_SYSCALL)
	{
		...
		switch (paramCount)
		{
			//two additional cases added to support more parameters
			case 5:
...
void InitialiseVGA()
{
	InitialiseSysCall(5, ConsoleClearScreenPix, 1);
	InitialiseSysCall(6, SetPixel, 3);
	InitialiseSysCall(7, BresenhamLine, 5);
	InitialiseSysCall(8, DrawRect, 5);
	InitialiseSysCall(9, FillRect, 5);
	InitialiseSysCall(10, DrawPolygon, 3);
	InitialiseSysCall(11, FillPolygon, 3);
	InitialiseSysCall(12, BresenhamDot, 5);
	InitialiseSysCall(13, DrawCircle, 4);
	InitialiseSysCall(14, FillCircle, 4);
